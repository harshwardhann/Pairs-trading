...
strikes=[]
currentstrike=strikes[0]

def orderBook(algo_client):
...
def priceDataExample(algo_client):
    ...
    instrument = algo_client.getExchangeInstrument(contract_name="ESM7", strike= currentstrike)  # future
...
def sendAlgoOrderExample(algo_client):
    ...
    order = algo_client.sendOrder(algo_name, user_parameters={"Instr": instrument.instrumentId})
